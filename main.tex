...
\section{Fundamentação Teórica}
    ...
    Para ilustrar as diferenças fundamentais entre as abordagens avaliadas, apresenta-se um cenário comum de desenvolvimento: a adição concorrente de dois métodos independentes na mesma região de uma classe Java.

    A Figura \ref{fig:cenario_input} apresenta o cenário de entrada: a versão Base (vazia) e as alterações realizadas nos ramos \textit{Left} (adição do método \texttt{sum}) e \textit{Right} (adição do método \texttt{sub}).
...
class MathUtils {
    int sub(int a, int b) {
        return a - b;
            ...
            \end{lstlisting}
        \end{minipage}
        \caption{Cenário de \textit{merge}: adição independente de métodos no mesmo local físico do arquivo.}
    ...
    O Git Merge (não estruturado) trata o arquivo como uma sequência de linhas. Como ambas as modificações ocorreram no mesmo ponto de inserção (dentro das chaves da classe), a ferramenta não consegue determinar a ordem correta e reporta um conflito, exigindo intervenção manual (Figura \ref{fig:resultado_git}).
 ...
 >>>>>>> branch-right
            ...
            \end{lstlisting}
        \end{minipage}
        \caption{Resultado do Git Merge: conflito textual reportado devido à sobreposição de linhas.}
        \label{fig:resultado_git}
    \end{figure}

    \subsubsection{Merge Estruturado (Spork)}
    O Spork (estruturado) constrói a Árvore Sintática Abstrata (AST) do código. Ele identifica que \texttt{sum} e \texttt{sub} são declarações de métodos distintos adicionadas ao nó da classe \texttt{MathUtils}. Como a ordem de métodos não afeta a semântica em Java, a ferramenta realiza a fusão automaticamente, produzindo um \textit{merge} limpo e correto (Figura \ref{fig:resultado_spork_sesame}).

    \subsubsection{Merge Semi-estruturado (SESAME)}
    O SESAME (semi-estruturado) utiliza separadores sintáticos (neste caso, as chaves \texttt{\{\}}) para inferir a estrutura. Ao pré-processar o texto inserindo quebras de linha nos separadores, ele isola os blocos de código. Isso permite que o algoritmo textual subjacente distinga as duas inserções como blocos independentes, alcançando o mesmo resultado de sucesso do Spork, mas com menor custo computacional (Figura \ref{fig:resultado_spork_sesame}).
...
class MathUtils {
    ...
    \subsubsection{Representação em Árvore (Estruturado/Spork)}
    Enquanto o Git vê apenas texto, o Spork converte o código numa Árvore Sintática Abstrata (AST). A Figura \ref{fig:ast_example} ilustra como a ferramenta "enxerga" a classe \texttt{MathUtils}. O conflito desaparece porque a adição do método \texttt{sum} (Left) e \texttt{sub} (Right) ocorre como novos nós filhos do nó \texttt{ClassDeclaration}, e não como linhas sobrepostas.
    ...
    \begin{figure}[ht]
        ...
        \node {ClassDeclaration \\ \textbf{"MathUtils"}}
            child { node {MethodDeclaration \\ \textbf{"sum"} \\ (Vindo de Left)} }
            child { node {MethodDeclaration \\ \textbf{"sub"} \\ (Vindo de Right)} };
        \end{tikzpicture}
        \caption{Representação simplificada da AST gerada por ferramentas estruturadas. As alterações são nós independentes na árvore, eliminando o conflito textual.}
        \label{fig:ast_example}
    \end{figure}

    \subsubsection{Representação com Separadores (SESAME)}
    O SESAME não constrói a árvore completa, mas "engana" o algoritmo textual inserindo marcadores artificiais. A Figura \ref{fig:sesame_intermediate} demonstra o estado intermediário do arquivo (Pós-processamento) antes da fusão.

    Observe os marcadores \texttt{\$\$\$\$\$\$} inseridos nos separadores \texttt{\{} e \texttt{\}}. Eles forçam o algoritmo textual a tratar as chaves e os corpos dos métodos como blocos distantes, permitindo que o \textit{diff3} insira ambos sem conflito.

    % --- CÓDIGO DO SESAME COM $$$$$$ ---
    \begin{figure}[ht]
        \centering
        % Left
        \begin{minipage}{0.45\textwidth}
            ...
            \begin{lstlisting}[basicstyle=\ttfamily\scriptsize, frame=single, backgroundcolor=\color{yellow!10}, showstringspaces=false]
class MathUtils
$$$$$$ {
$$$$$$
    int sum(int a, int b)
...
$$$$$$ }
            \end{lstlisting}
        \end{minipage}
        \caption{Visualização do mecanismo do SESAME: inserção de quebras de linha e \textit{placeholders} (\texttt{\$\$\$}) nos separadores sintáticos para isolar os blocos de código.}
